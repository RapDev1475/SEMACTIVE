...
export default function MouvementsPage() {
  ...
  function mapTypeToConstraint(typeNom: string): string {
    ...
    if (standardMappings[typeNom]) return standardMappings[typeNom]
    ...
    if (lowerStandardMappings[lowerNom]) return lowerStandardMappings[lowerNom]
    if (typeNom === 'Transfert_Stock') return typeNom
    return typeNom
  }
    ...
    try {
      if (isTransfertEntreTechniciens() && mouvementData.personne_source_id) {
        const seriesInStock = stockTechnicienSource.filter(s =>
          s.article_id === articleId && s.numero_serie_id !== null
        )
  ...
  function ajouterLigneAuto(article: Article, numeroSerie?: any) {
    ...
    if (isTransfertEntreTechniciens() && mouvementData.personne_source_id) {
      const articlesDisponibles = stockTechnicienSource.map(s => s.article).filter(Boolean)
      const uniqueArticles = Array.from(new Map(articlesDisponibles.map((a: any) => [a.id, a])).values())
      setArticles(uniqueArticles as Article[])
    } else {
      fetchArticles()
    }
  ...
  async function validerMouvement() {
    ...
    const lignesAMettreAJour = [...lignesMouvement].map(l => ({...l}))
  ...
  // Filtrer les articles pour le Select
            ...
            <CardContent>
              <form onSubmit={ajouterLigne} className="space-y-6">
                ...
                {ligneFormData.article_id && (() => {
                  ...
                  if (!art) return null
                  return (
                    <div className="p-6 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-950 dark:to-indigo-950 rounded-xl border-2 border-blue-300">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex gap-4 mt-1 text-sm text-blue-700 dark:text-blue-300">
                            <span className="font-mono">N°: {art.numero_article}</span>
                            <span>•</span>
                            <span>Stock disponible: {art.quantite_stock}</span>
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                {ligneFormData.article_id && mouvementData.type_mouvement?.toLowerCase().includes('reception') && (
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <Label className="text-base mb-2 block">Nouveau N° de série (à créer)</Label>
                      <Input
                        className="h-14 text-lg"
                    ...
                    <div>
                      <Label className="text-base mb-2 block">Nouvelle Adresse MAC (facultatif)</Label>
                      <Input
                        className="h-14 text-lg"
                ...
                {ligneFormData.article_id && numerosSerieDisponibles.length > 0 && (
                  <div>
                    <Label className="text-base mb-2 block">
                      {mouvementData.type_mouvement?.toLowerCase().includes('reception') 
                        ? 'Ou sélectionner un N° existant'
                        : 'Numéro de série / MAC'
                      }
                    </Label>
                    <Select 
                ...
                <table className="w-full">
                  ...
                  <tbody>
                    {lignesMouvement.map((ligne, idx) => {
                      const numeroSerieAffiche = ligne.nouveau_numero_serie || ligne.numero_serie || '-'
                      const adresseMacAffiche = ligne.nouvelle_adresse_mac || ligne.adresse_mac || '-'
                      return (
